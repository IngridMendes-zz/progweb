...
class AlunoController extends Controller
{
    ...
    public function actionView($id='1551')
    {
        $a= new Aluno();

        $ano_ingresso =  Aluno::find()->where(['id' => $id])->one()['ano_ingresso'];

        $numero = $a::find()->where('ano_ingresso='.$ano_ingresso)->count();
        $data = date("Y");
        return $this->render('view', [
            'model' => $this->findModel($id), 'data'=>$data, 'numero'=>$numero
        ]);
    }
